...
uint32_t dev_pciReadWord( uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset ) {
    uint32_t address;
...
uint32_t dev_pciReadDWord( uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset ) {
    uint32_t address;
    uint32_t lbus  = (uint32_t)bus;
    uint32_t lslot = (uint32_t)slot;
    uint32_t lfunc = (uint32_t)func;
    ...
    // Write out the address
	__asm__ __volatile__("mov 0xCF8, %%dx;movl %0, %%eax;out %%eax, %%dx;"
		:
		:"rm"(address)
		:"eax", "dx"
		);
    ...
    // Read in the data, max offset - 2, odd offsets ignored, offset > 4 case above
	__asm__ __volatile__("movw 0xCFC, %%dx;in %%dx, %%eax;movl %%eax, %0;"
		:"=rm"(tmp)
		:
		: "eax", "dx"
     );
...
uint32_t dev_pciGetVendor(uint8_t bus, uint8_t slot) {
...
uint16_t dev_pciGetClass(uint8_t bus, uint8_t slot) {
	return dev_pciReadWord(bus, slot, 0x0, 8);
}
